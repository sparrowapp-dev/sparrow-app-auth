<script lang="ts">
	...
	onMount(async () => {
		...
		if (res?.data?.accessToken?.token) {
			const { accessToken, refreshToken, teamId } = res.data;

			const deepLink = `sparrow://invite-login?accessToken=${accessToken.token}&refreshToken=${refreshToken.token}&teamId=${teamId}`;
