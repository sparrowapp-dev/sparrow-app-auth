				...
				if (!validationErrors?.email && !validationErrors?.password) {
					...
					if (result.isSuccessful) {
						...
						if (response?.isEmailVerified) {
							...
							const sparrowRedirect = `sparrow://?selfHostbackendUrl=${constants.APP_EDITION === AppEdition.SELFHOSTED ? constants.API_URL : ""}&accessToken=${accessToken}&refreshToken=${refreshToken}&response=${JSON.stringify(response)}&event=login&method=email`;
